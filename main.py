...
cap = cv2.VideoCapture("Test Videos/ehsan.mp4")

target_classes = ['macan']
...
# Function untuk menggambar polygon(roi)
...
# Function untuk mengecek titik didalam polygon
    ...
    for index, row in results.pandas().xyxy[0].iterrows():
        ...
        if len(pts) >= 4:
            ...
            if center_x is not None and center_y is not None:
                ...
                #Memeriksa apakah pusat objek berada di dalam poligon dan apakah objeknya adalah hewan
                if inside_polygon((center_x, center_y), np.array([pts])) and name == ' ':
                    ...
                    if count < number_of_photos:
                        cv2.imwrite("D:\CAPSTONE 6\model-yolov5-main\Test Videos" + str(count) + ".jpg", frame_detected)
                    # Playing the alarm
                    if not pygame.mixer.music.get_busy():
                        pygame.mixer.music.play()
                        alarm_playing = True
                    cv2.putText(frame, "Target", (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    cv2.putText(frame, "Target Detected", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
